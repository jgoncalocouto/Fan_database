...
def fan_working_point(fan_name='9CRA0612P0S001',N_fan=1,type='Inlet',entry_geometry='Concealed',number_of_inlets='Single',filter='G4',area='Standard',N_filter=3.5, fan_details='3 fans of φ160 and W=51'):
    import pandas as pd
    import numpy as np
    ...
    list = pd.DataFrame(
        {'rpm': df_fan['Rotational Speed - [rpm]'].unique(), 'flowrate': np.nan, 'static pressure': np.nan})
    ...
    df_inst_selected = df_installation[
        ...
        (df_installation['Fan Array Details'] == fan_details)
    ...
    for i in range(len(list['rpm'])):
        ind = i
        fan_flow = df_fan[df_fan['Rotational Speed - [rpm]'] == list['rpm'].loc[ind]][
                       'Flowrate - [m^3/min]'] * 60 * N_fan
        ...
        if ind == 0:
            plt.plot(Vdot, inst_psat, label=('Installation Code = '+df_inst_selected['Number'].values[0]))

        plt.plot(Vdot, P_fan, label=('Fan Designation:' + str(fan_name) + '; N = ' + str(N_fan) + '; RPM=' + str(list['rpm'].loc[ind])))
